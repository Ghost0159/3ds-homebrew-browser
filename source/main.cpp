...
namespace {
std::array<hbb::ListingMetadata, 3> get_title_list_draw_state(
    hbb::FilteredListCursor const& cursor,
    std::array<hbb::AppInfo, 3> const& current_page_app_info) {
  bool const there_are_no_titles = cursor.begin == cursor.end;
  hbb::ListingMetadata const hidden{hbb::ListingTitleDisplay::kHidden, nullptr, "", ""};
  ...
  std::array<hbb::ListingMetadata, 3> visible_titles{{hidden, hidden, hidden}};
  std::transform(visible_titles_begin, visible_titles_end,
      begin(visible_titles), [](hbb::Title const* title) {
    return hbb::ListingMetadata{hbb::ListingTitleDisplay::kVisible, nullptr, title->title_name, ""};
  });
...
hbb::ListingScrollbar get_scrollbar_draw_state(hbb::FilteredListCursor const& cursor, bool active) {
  ...
  if (list_size < 3) {
    return {hbb::ListingScrollbarDisplay::kHidden, 0, active};
  }
  return {
      hbb::ListingScrollbarDisplay::kVisible,
...
void initialize_sockets() {
  ...
  if (bank == nullptr) {
    hbb::debug_message("memalign failed");
    return;
  }

  u32 ret = SOC_Initialize(bank, 0x48000);
  if (ret) {
    hbb::debug_message("Error initializing sockets: " + hbb::string_from<u32>(ret, true));
  }
}
...
int main()
{
  ...
  std::tie(error, state.full_homebrew_list) = hbb::get_homebrew_listing(hbb::kServer);
  hbb::debug_message("Downloaded " + hbb::string_from<int>(state.full_homebrew_list.size()) + " titles!");
  ...
  while (aptMainLoop())
  {
    ...
    if (state.filtered_list_dirty) {
      ...
      update_metadata_for_page(hbb::kServer, get_title_list_cursor(state.filtered_homebrew_list,
